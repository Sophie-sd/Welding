...
echo "  3. SSL (webroot — nginx can keep running on :80):"
echo "       mkdir -p /var/www/certbot"
echo "       # ensure USE_HTTPS=false first deploy so HTTP serves ACME"
echo "       bash deploy/docker/ssl-renew.sh"
echo "       # set USE_HTTPS=true in .env"
echo "       bash deploy/docker/deploy.sh"
echo "  4. Superuser:"
echo "       docker compose exec web python manage.py createsuperuser"
echo "  Auto-renew: certbot.timer + deploy/docker/ssl-renew.sh (webroot)."
